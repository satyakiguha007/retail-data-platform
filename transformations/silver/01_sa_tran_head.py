...
# MAGIC 1. **Surrogate from source-unique ID** — `TRAN_SEQ_NO = xxhash64(rtlog_orig_sys, tran_seq_no_natural)`. The natural composite `(store, date, register, tran_no)` is NOT unique under fault injection — Module 1's fault layer deliberately duplicates `tran_no` (fault type `TRAN_NO_DUP`). The simulator's own `tran_head.tran_seq_no` stays unique because the fault injector touches `tran_no`, not `tran_seq_no`. We hash the source's own unique ID with a channel namespace.
...
# MAGIC 2. **Surrogate key** — `TRAN_SEQ_NO = xxhash64(rtlog_orig_sys, tran_seq_no_natural)`. Hashes the source's own unique ID, not the natural composite (which can collide under fault injection).
...
def merge_microbatch(microBatchDF: DataFrame, batch_id: int) -> None:
    ...
    flat = (
        ...
        .filter(
            ...
            & col("TRAN_SEQ_NO_NATURAL").isNotNull()   # required — basis of the surrogate
        )
    )

    # 2. Surrogate key — hash the source's natural unique ID (not the composite).
    # Composite (store, date, register, tran_no) is non-unique under fault injection.
    keyed = flat.withColumn(
        "TRAN_SEQ_NO",
        xxhash64(col("RTLOG_ORIG_SYS"), col("TRAN_SEQ_NO_NATURAL")),
    )
